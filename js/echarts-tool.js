...
function weibo_wbzl() {
    ...
    // 模拟后端数据
    var jsonData = [
        {'郴州':[120, 132, 101, 134, 90, 230, 210]},
        {'东江湖':[220, 182, 191, 234, 290, 330, 310]},
        {'仰天湖':[150, 232, 201, 154, 190, 330, 410]},
        {'大草原':[320, 332, 301, 334, 390, 330, 320]},
        {'高椅岭':[820, 932, 901, 934, 1290, 1330, 1320]},
    ]
    // 解析 JSON 数据
    // 词条名称
    var names = jsonData.map(item => Object.keys(item)[0]);
    // 对应数值
    var data = jsonData.map(item => Object.values(item)[0]);

    // 动态生成 series 数组
    const series = names.map((city, index) => ({
        name: city,
        type: 'line',
        stack: 'Total',
        data: data[index]
    ...
    // x轴标签值
    var xlabel = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
...
function weibo_wxyg(aa) {
    ...
    // 解析 JSON 数据
    var jdata = jsonData[0]; // 取第一个对象
    var xlabel = Object.keys(jdata);
    var data = Object.values(jdata);
    // var xlabel = ['帖子数', '评论数', '点赞数'];
    // var data = [600, 200, 900];
...
function weibo_wwbzl() {
    ...
    var data = jsonData.map(item => {
        var key = Object.keys(item)[0];  // 获取对象的键（即名称）
        var value = item[key];           // 获取对应的值
        return { name: key, value: value };  // 返回新的对象格式
    });

    /********** 提取value 归一化处理 控制气泡大小 start **********/
    // 将字符串类型的数值转换为数字类型
    data.forEach(item => item.value = Number(item.value));
    // 找到最大值和最小值
    ...
    // 将值归一化到 50 到 100 的范围，并提取归一化后的数值（浮点数）
    var normalizedValues = data.map(item =>
        // parseFloat(normalize(item.value, minValue, maxValue, 1, 100).toFixed(2)) // 保留两位小数
        Math.round(normalize(item.value, minValue, maxValue, 50, 100))
    );
    console.log(normalizedValues);
    // 设置气泡大小
    var symbolSizeData = normalizedValues;
    /********** 提取value 归一化处理 控制气泡大小  end **********/

    // 气泡显示位置  x小靠左 y小靠下
    var offsetData = [[80, 63], [35, 73], [30, 33], [60, 33], [10, 20]];
...
function gov_yqbj() {
    ...
    var xlabel = jsonData.map(item => Object.keys(item)[0]);
    // 对应数值
    var data = jsonData.map(item => Object.values(item)[0]);

    var clist = ["#37b70c", "#dd3f36", "#d0a00e"];


    // var xlabel = ['09月', '10月', '11月']
    // var data = [[0, 0, 100], [1, 0, 20], [2, 0, 60],]
...
// 右下 日报社板块 饼图 + 折线图
function gov_xinwenwang() {
    var chartDom = document.getElementById('wenlvju');
    ...
    var data = [
        ...
        {'时政': [56.5, 82.1, 88.7, 70.1, 53.4, 89.1]},
        {'民生': [51.1, 51.4, 55.1, 53.3, 73.8, 68.7]},
        {'专题': [40.1, 62.2, 69.5, 36.4, 45.2, 32.5]},
        {'旅游': [25.2, 37.1, 41.2, 18, 33.9, 49.1]}
    ];
    // 转换成 ECharts 需要的 source 格式
    ...
    option = {
        ...
        yAxis: {
            gridIndex: 0,
            axisLabel: {show: true, textStyle: {color: "rgb(255,255,255)", fontSize: '12',},},
            axisTick: {show: false,},
            axisLine: {show: true, lineStyle: {color: "rgba(255,255,255,.1    )", width: 1, type: "solid"},},
            splitLine: {lineStyle: {color: "rgba(255,255,255,.1)",}}
        },
